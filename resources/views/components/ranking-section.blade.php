            ...
            <div class="ranking-header">
                @if($index === 0)
                    <div class="crown-container">
                        ...
                        <div class="ribbon">{{ $ranking['name'] }}</div>
                    </div>
                @else
                    <span class="rank-number rank-{{ $index + 1 }}">{{ $index + 1 }}位</span>
                    <span class="site-name">{{ $ranking['name'] }}</span>
                @endif
            </div>
            <div class="ranking-content">
                @if(isset($ranking['ad_code']))
                ...
                <div class="ranking-article">
                    <div class="ranking-article-nepali">
                        @if(isset($ranking['description_nepali_short']))
                            <span class="ranking-article-nepali-short" id="desc-nepali-short-{{ $index }}">
                                {!! nl2br(e($ranking['description_nepali_short'])) !!}
                            </span>
                            <a class="read-more-link" onclick="toggleDescription({{ $index }})" id="read-more-{{ $index }}">
                                続きを読む
                            </a>
                            <span class="ranking-article-nepali-full" id="desc-nepali-full-{{ $index }}">
                                {!! nl2br(e($ranking['description_nepali'])) !!}
                            </span>
                        @else
                            {{ $ranking['description_nepali'] ?? '' }}
                        @endif
                    </div>
                </div>
            </div>
