...
<div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold">職務経歴書詳細</h2>
    ...
    <div class="space-y-6">
        ...
        @if($careerHistorySubmission->job_summary)
            <div class="border rounded-lg p-4">
                <h3 class="text-lg font-semibold mb-4 border-b pb-2">職務要約</h3>
        ...
        <!-- 職務経歴 -->
        <div class="border rounded-lg p-4">
            <h3 class="text-lg font-semibold mb-4 border-b pb-2">職務経歴</h3>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                    ...
                                    @if(!empty($career['business_content']))
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">事業内容</label>
                                            <p class="text-sm">{{ $career['business_content'] }}</p>
                                        </div>
                                    @endif
                                    @if(!empty($career['employee_count']))
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">従業員数</label>
                                            <p class="text-sm">{{ $career['employee_count'] }}</p>
                                        </div>
                                    @endif
                                    @if(!empty($career['capital']))
                                        <div>
                                            <label class="block text-xs font-medium text-gray-600 mb-1">資本金</label>
                                ...
                                @if(!empty($career['job_description']))
                                    <div>
                                        <label class="block text-xs font-medium text-gray-600 mb-1">職務内容</label>
            ...
            @else
                <p class="text-gray-500 text-sm">職務経歴情報がありません</p>
            @endif
        </div>
        ...
        @if($careerHistorySubmission->self_pr)
            <div class="border rounded-lg p-4">
                <h3 class="text-lg font-semibold mb-4 border-b pb-2">自己PR</h3>
