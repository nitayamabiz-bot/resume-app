...
<div class="w-full bg-white rounded-lg shadow-md p-6">
    ...
    <?php if($data): ?>
        ...
        <div class="space-y-6">
            ...
            <div class="border-b pb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        <span class="text-gray-600">氏名（カタカナ） / नाम (カタカナ):</span>
        ...
        function generatePdf() {
            ...
            button.innerHTML = '生成中... / PDF बनाउँदै...';
            
            const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || 
                             document.querySelector('input[name="_token"]')?.value || 
                             '<?php echo e(csrf_token()); ?>';
            
            fetch('<?php echo e(route("resume.generate-pdf")); ?>', {
                method: 'POST',
                headers: {
            ...
            .then(response => {
                ...
                if (!response.ok) {
                    return response.json().then(data => {
                        ...
                        throw new Error('PDFの生成に失敗しました（' + response.status + '）');
                    });
                }
            ...
            .then(data => {
                ...
                if (!data.pdf) {
                    ...
                    throw new Error('ファイルデータが含まれていません');
                }
                ...
                try {
                    ...
                    // ファイルタイプに応じてMIMEタイプを設定
                    const fileType = data.file_type || 'pdf';
                    const mimeType = fileType === 'excel' ? 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' : 'application/pdf';
                    const blob = new Blob([bytes], { type: mimeType });
                    
                    console.log(fileType === 'excel' ? 'Excel' : 'PDF', 'blob created:', blob.size, 'bytes');
                    
                    // ファイルをダウンロード
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = data.filename || (fileType === 'excel' ? '履歴書_' + new Date().getTime() + '.xlsx' : '履歴書_' + new Date().getTime() + '.pdf');
                    document.body.appendChild(a);
                    a.click();
                    window.URL.revokeObjectURL(url);
                    document.body.removeChild(a);
                    
                    console.log(fileType === 'excel' ? 'Excel' : 'PDF', 'download initiated');
                    
                    // Excelファイルの場合、メッセージを表示
                    if (fileType === 'excel' && data.message) {
                        alert(data.message);
                    }
                } catch (decodeError) {
                    console.error('File decode error:', decodeError);
                    throw new Error('ファイルデータのデコードに失敗しました: ' + decodeError.message);
                }
            ...
            .catch(error => {
                ...
                let errorMessage = 'PDFの生成に失敗しました: ' + error.message;
                if (error.message.includes('テンプレート') || error.message.includes('template')) {
                    errorMessage = error.message; // サーバーからの詳細メッセージをそのまま表示
                }
                alert(errorMessage);
                button.disabled = false;
                button.innerHTML = originalText;
...
<?php /**PATH /Users/ni/Desktop/hamro-life-japan.com/resources/views/resume/confirm.blade.php ENDPATH**/ ?>